...
//                     document.cookie = `jwt=${logIn.data.session.access_token}; path=/; max-age=${60 * 60 * 24 * 7}; HttpOnly; SameSite=Lax`;
//                     document.cookie = `email=${email}; path=/; max-age=${60 * 60 * 24 * 7}; httpOnly; SameSite=Lax`;
//                 } else {
...
import { useState } from "react";
import { validateEmail, validatePassword } from "@/app/utils/helper";
import { fetchSignIn, validateToken, saveTokenOnServer } from "@/app/lib/api"; // נוסיף פונקציה חדשה לשמירה בשרת
import { useRouter } from "next/navigation";
...
export default function SignIn({ toggleSignUp }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (logIn.success) {
                // Start by checking the token validity before proceeding with saving
